...
export REPOSITORY="borg@10.200.0.220:/var/backup/{hostname}"
...
mysqldump -v -h127.0.0.1 -umysqluser -pmysqlpass joomla | gzip > /tmp/joomla.gz



# Backup all of /home except a few excluded directories and files
borg create -v --stats --compression lz4                 \
    $REPOSITORY::'{hostname}-{now:%Y-%m-%d_%H:%M:%S}' /etc /tmp/joomla.gz \
