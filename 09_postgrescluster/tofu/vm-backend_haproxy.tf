...
resource "yandex_compute_instance" "nodehaproxybackend" {
  depends_on = [resource.yandex_compute_instance.bastion]
  allow_stopping_for_update = true
  name = "otus-backend-haproxy-${count.index + 1}"
  hostname="otus-backend-haproxy-${count.index + 1}"
  ...
  boot_disk {
    initialize_params {
      name     = "boot-disk-backend-haproxy-${count.index + 1}"
      size     = "10"
